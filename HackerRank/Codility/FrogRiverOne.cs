...
{
    public class FrogRiverOne : IExcercise
    {
        /// <summary>
...
N and X are integers within the range [1..100,000];
        ...
        {
            var positionTime = new Dictionary<int, int>();
            for (int i = 0; i < A.Length; i++)
            {
                if (!positionTime.ContainsKey(A[i]))
                    positionTime.Add(A[i], i);
            }
            var earlietTime = -1;
            for (int x = 1; x <= X; x++)
            {
                if (!positionTime.ContainsKey(x))
                    return -1;
                if (positionTime[x] > earlietTime)
                    earlietTime = positionTime[x];
            }
